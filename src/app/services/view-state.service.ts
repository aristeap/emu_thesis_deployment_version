...
class ViewStateService{
	...
	initialize() {
		...
		this.states.labeling = {
			'permittedActions': ['zoom', 'playaudio', 'spectSettingsChange', 'addLevelSegBtnClick', 'addLevelPointBtnClick','deleteLevelBtnClick', 'renameSelLevelBtnClick', 'addSpeakerBtnClick','renameSpeakerBtnClick','downloadTextGridBtnClick', 'downloadAnnotationBtnClick', 'spectSettingsChange', 'clearBtnClick', 'labelAction', 'toggleSideBars', 'saveBndlBtnClick', 'showHierarchyBtnClick', 'editDBconfigBtnClick', 'aboutBtnClick', 'searchBtnClick']
		};
		this.states.nonAudioDisplay = {
			'permittedActions' : []
		};
	...
	*/
	public getPermission(actionName) {
		//  console.log("isnide getPermission of view-state.service.ts");
		//  console.log("actionName: ",actionName);
		// if(actionName === 'addSpeakerBtnClick'){
		// 	console.log("the addSpeakerBtn is asking for permission????????????????????????????????????????????????????");
		// }
		//  console.log("returns: ",this.curState.permittedActions.indexOf(actionName) > -1);
	...
	public getCurStateName(): string {
		for (const stateName in this.states) {
		  if (this.states.hasOwnProperty(stateName)) {
			if (this.states[stateName] === this.curState) {
			  return stateName;
			}
		  }
	...
	*/
	public setcurClickLevel(levelID, levelType, levelIndex) {
		// console.log("view-state.service.ts-> setcurClickLevel()");

		this.setcurClickLevelName(levelID, levelIndex);
		this.setcurClickLevelType(levelType);
		// console.log("this.setcurClickLevelName(levelID, levelIndex): ",this.setcurClickLevelName(levelID, levelIndex));
		// console.log("this.setcurClickLevelType(levelType): ",this.setcurClickLevelType(levelType));
	...
	*/
	public setcurClickItem(item) {
		// console.log("inside view-state.service.ts-> setcurClickItem(item)");
