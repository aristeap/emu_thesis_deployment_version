...
class DrawHelperService{
	...
	public calculateOsciPeaks () {
		...
		if (!this.SoundHandlerService.audioBuffer || typeof this.SoundHandlerService.audioBuffer.getChannelData !== 'function') {
			console.warn("No valid audioBuffer to calculate peaks. Skipping...");
			return;
		}
	...
	public freshRedrawDrawOsciOnCanvas(canvas, sS, eS, forceToCalcOsciPeaks) {
		// console.log("inside draw-helper.service.ts->freshRedrawDrawOsciOnCanvas()");

		 // If no real audio buffer, skip the osci drawing logic.
		 if (!this.SoundHandlerService.audioBuffer ||typeof this.SoundHandlerService.audioBuffer.getChannelData !== 'function') {
			console.warn("No valid audioBuffer to draw OSCI. Skipping...");
			return;
		}
